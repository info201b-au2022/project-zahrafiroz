...
#Establishes where this information is coming from
source("../source/summary_info.R")
# Pulls the values we want to put into text:
...
# We then use these values by referencing it within our text = `r <variable>` example: `r one` and it will show the actual value once knit
```
...
knitr::kable(data_table) #Tell the program to use the specific function from a specific package
```
...
hazards_chart # This is the variable that the chart is saved in within the file referenced as the source for this code chunk
```
