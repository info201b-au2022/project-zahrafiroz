...
hazard_groups <- climate_hazards %>%
  ...
  mutate(vulnerable_pop = str_trim(vulnerable_pop, "left")) %>%
  count(vulnerable_pop) %>%
  filter(!vulnerable_pop %in% c("Question not applicable", "Other, please specify", "Do not know")) %>%
  mutate(vulnerable_pop = str_replace(vulnerable_pop, "Personas que viven en viviendas de calidad inferior.", "People living in substandard housing")) %>% 
  arrange(-n) %>% 
  head(10)
...
top_disasters <- disaster_freq %>%
  ...
  group_by(Country) %>%
  mutate(disaster_sum = sum(across("F1980":"F2001"), na.rm = TRUE)) %>%
