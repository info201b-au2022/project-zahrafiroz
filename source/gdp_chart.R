...
disaster_freq_test <- disaster_freq %>% 
...
View(disaster_freq_test)

gdp <- gov_expend %>% 
  filter(Unit == "Percent of GDP") %>% 
  group_by(Country) %>%
  #select(-starts_with("F200"), -starts_with("F19")) %>% 
  summarise(sum = sum(across(starts_with("F")), na.rm = T)) %>% 
  mutate(avg_gdp = sum / ncol(select(gov_expend, starts_with("F")))) %>% 
  select(-sum)

gdp_and_disasters <- left_join(gdp, disaster_freq_test, by = "Country")

gdp_chart <- plot_ly(data = gdp_and_disasters,
              x = ~total_disasters,
              y = ~avg_gdp,
              type = "scatter",
              text = ~n,
              textposition = "auto",
              hoverinfo = "text",
              hovertext = paste0(gdp_and_disasters$Country, 
                                 "<br>", "Disasters: ",
